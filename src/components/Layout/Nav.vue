...
<template>
  <nav :class="['absolute top-0 left-0 w-full z-50 py-4 px-6', {'bg-[#0a0a0a]': isMenuOpen}]">
    ...
    <div v-if="isMenuOpen" class="lg:hidden mt-4 space-y-4">
      ...
      <template v-else>
        <RouterLink to="/login" class="block nav-link-login text-green-500 px-4 py-2 rounded " @click="toggleMenu">Login</RouterLink>
        <RouterLink to="/register" class="block nav-link-register text-yellow-500 px-4 py-2 rounded" @click="toggleMenu">Register</RouterLink>
      </template>
    </div>
  </nav>
</template>
