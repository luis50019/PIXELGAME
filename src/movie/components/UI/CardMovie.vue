...
<template>
  <div class="card-pelicula  flex flex-col gap-3">
    ...
    <div class="cont-info flex flex-col justify-between gap-2 p-2">
      <h3 class="text-wrap text-center text-4xl max-h-24 min-h-24 font-bold text-white">{{ movie.title }}</h3>
      <p class="max-h-64 overflow-y-auto contenedor mb-10">{{ movie.overview?movie.overview:"Sin descripcion" }}</p>
      <RouterLink 
        :to="{ name: 'movie', params: { id: movie.id || null }}" 
